...
const DeliveryPartnerFAQ = () => {
  const [faqs, setFaqs] = useState([]);

  const fetchSettings = async () => {
    const response = await getSettingContent();
    if (response.type === "success") {
      setFaqs(response.setting.deliveryPartnerFaqs || []);
    }
  };
  ...
  const handleSave = async () => {
    try {
      const response = await updateSettingContent({ deliveryPartnerFaqs: faqs });
      if (response.type === "success") {
  ...
  return (
    <div className="page-content d-flex flex-column gap-4">
      <h4><i className="bx bx-help-circle" /> Delivery Partner FAQs</h4>
      <Form
        onSubmit={(e) => {
          e.preventDefault();
          handleSave();
...
export default DeliveryPartnerFAQ;
